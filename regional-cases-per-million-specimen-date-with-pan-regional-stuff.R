...
casesbyregion<- cases %>% select (-areaCode, -areaType, -cumCasesBySpecimenDate) %>% pivot_wider(names_from = areaName, values_from=newCasesBySpecimenDate, values_fn=mean)
casesbynation<- nations %>% select (-areaCode, -areaType, -cumCasesBySpecimenDate) %>% pivot_wider(names_from = areaName, values_from=newCasesBySpecimeDate, values_fn=mean)
...
labs(title="The North (Specimen Date)", x="Date", y="Cases/million (log scale)")
#+
...
labs(title="The South (Specimen Date)", x="Date", y="Cases/million (log scale)")
#+
# geom_smooth(method="lm", se=F, lwy=1) 
dev.off()
...
labs(title="Scotland/NI (Specimen Date)", x="Date", y="Cases/million (log scale)")+
 geom_smooth(method="lm", se=F, lwy=1) 
dev.off()
