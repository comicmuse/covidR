...
td2=seq(as.Date("2021-02-15"), as.Date("2021-04-15"), "days")
...
#Required Rate
reqrate=round(tail(coredata(peopleleft),1) / as.numeric(as.Date("2021-04-15") - end(peopleleft)))

#Merge all the timeseries into one zoo
finalzoo=merge.zoo(target, target2, peopleleft, burndownzoo, legacy, all=T, drop=F)

#Draw the graph
plot.zoo(finalzoo/1000000, plot.type="single", col=c(4,4,3,8,5), lty=c(1,1, 1,5), lwd=2, xlab="Date",ylab="People (millions)", main="Vaccine Burndown Priority Groups 1-9 (First Dose)")
